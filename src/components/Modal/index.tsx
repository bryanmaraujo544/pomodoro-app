...
export const Modal = ({
  ...
  useEffect(() => {
    axios.get('https://www.googleapis.com/youtube/v3/search?part=snippet&key=AIzaSyCWrEZ2sdh-zbgLGCLvXvC_zHKkYvHQWe0&type=video&&maxResults=2&q=codeandchill').then(({ data }) => {
      setItems(data.items);
      setIsLoading(false);
    }).catch((error) => {
